...
# Filtra o DataFrame para manter apenas as linhas com os maiores volumes
...
for acao in df_top10["ticker"]:
    ...
    abertura = df_preco.iloc[0, -2]  # Pegar o valor da primeira linha

    # pegar o valor de fechamento na data final do período:
    fechamento = df_preco.iloc[-1, -1]  # Pegar o valor da última linha

    # cálculo do retorno (é o que usarei para comparar com a IBOVESPA)
    retorno = ((fechamento - abertura) / abertura) * 100
    
    # adiciona no df a coluna retorno
    df_preco["retorno"] = retorno 

    # pega só a última linha, que é o valor de abertura e fechamento no último dia
...
# Ranking das ações (earning yield e roic): novas colunas no data frame com os respectivos rankings

# top30 roic: 
df_top30_roic = df_comparacao.nlargest(35, "roic").reset_index(drop=True)
df_top30_roic.index = range(1, len(df_top30_roic) + 1)
df_top30_roic["index_roic"] = df_top30_roic.index

# top 30 ey:
df_top30_ey = df_comparacao.nlargest(35, "earning_yield").reset_index(drop=True)
df_top30_ey.index = range(1, len(df_top30_ey) + 1)
df_top30_ey["index_ey"] = df_top30_ey.index

# juntar numa data frame o ranking (do ey e do roic) e o nome das ações: 
df_merged  = pd.merge(df_top30_ey[["ticker", "index_ey", "volume"]], 
                      df_top30_roic[["ticker", "index_roic"]], 
                      on="ticker", 
                      how="inner")

# ranking médio: (soma das posições)
...
df_merged['ticker_base'] = df_merged['ticker'].str[:3]  # identificar as ações repetidas
...
df_final_sorted = df_final_sorted.nsmallest(10, "media")  # só os top10
...
# df com os tickers do top 10 Magic Formula e seus respectivos retornos; ignore index para sair de 0 - 9
...
# formatar para sair em porcentagem:
retorno_formatado_magic = f"{soma_total_magic:.2f}%"  # retorno da carteira de ações do top10 Magic Formula
...
print(df_final)
print("Retorno da carteira:", retorno_formatado_roe)

# MAGIC FORMULA:
print("Carteira de ações de acordo com a Magic Formula:")
print(df_final_magic)
print("Retorno da carteira:", retorno_formatado_magic)
